...
export default class SwipeUpDown extends Component {
  ...
  static propTypes = {
    ...
    itemMini: PropTypes.element,
    itemFull: PropTypes.element.isRequired,
  ...
  _onPanResponderMove (event, gestureState) {
    if (gestureState.dy > 0 && !this.checkCollapsed) {
      ...
      this.swipeIconRef && this.swipeIconRef.setState({ icon: icons.minus })
      ...
      this.swipeIconRef &&
      this.swipeIconRef.setState({ icon: icons.minus, showIcon: true })
  ...
  showFull () {
    ...
    this.swipeIconRef &&
    this.swipeIconRef.setState({ icon: icons.arrowDown, showIcon: true })
    this.updateNativeProps()
    this.state.collapsed && this.setState({ collapsed: false })
